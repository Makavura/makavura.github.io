class BannerLink extends HTMLElement {
  ...
  connectedCallback() {
    this.innerHTML = `<div class="px-1 py-1 bg-[#233DFF] rounded-md ">
                <div class="drop-shadow-xl shadow-2xl  bg-white  p-2 rounded-md">
                    <div class="grid grid-cols-2 items-center place-content-between">
                        <p class="aloja text-slate-400 text-2xl inline">${this.getAttribute(
                          "bannerPath"
                        )}</p>
...
${
  this.hasAttribute("svg")
    ? `<img src="${this.getAttribute("svgIcon")}" class=" hidden" />`
    : null
}
                </div>
            </div>`;
